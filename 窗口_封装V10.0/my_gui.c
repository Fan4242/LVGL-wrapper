...
void my_gui(void)
{
    ...
    my_str_win_t my_str_win = {0};
    my_str_win.header_height = scr_act_height()/12;
    my_str_win.width = scr_act_width()*5/8;
    my_str_win.height = scr_act_height()*4/7;
    my_str_win.position = LV_ALIGN_CENTER;
    my_str_win.border_width = 1;
    my_str_win.border_color = 0x8a8a8a;
    my_str_win.border_opa = 100;
    my_str_win.border_radius = 10;
    win = my_win_init(act,act,&my_str_win);

    lv_obj_t *btn_setting = my_win_add_btn(win,LV_SYMBOL_SETTINGS,30,0,0,0x000000,0x1000000);
    lv_obj_t *title = lv_win_add_title(win, "Setting");
    lv_obj_t *btn_close = my_win_add_btn(win,LV_SYMBOL_CLOSE,30,0,0,0x000000,0xff0000);
    ...
    my_str_slider.width = scr_act_width()/3;
    my_str_slider.height = scr_act_height()/30;
    my_str_slider.position = LV_ALIGN_CENTER;
    my_str_slider.x_ofs = 15;
    my_str_slider.y_ofs = -scr_act_height()/14;
    my_str_slider.main_color = 0x787c78;
    my_str_slider.indicator_color = 0xc3c3c3;
    my_str_slider.konb_disable = 1;
    my_str_slider.right_value = 50;
    lv_obj_t *slider_audio = my_slider_init(content,content,&my_str_slider);

    my_label_text_create(content,slider_audio,LV_ALIGN_OUT_LEFT_MID, -scr_act_width()/40, 0,LV_SYMBOL_AUDIO,&lv_font_montserrat_16);

    my_str_slider.y_ofs = scr_act_height()/14;
    lv_obj_t *slider_bell = my_slider_init(content,content,&my_str_slider);
    my_label_text_create(content,slider_bell,LV_ALIGN_OUT_LEFT_MID, -scr_act_width()/40, 0,LV_SYMBOL_BELL,&lv_font_montserrat_16);
