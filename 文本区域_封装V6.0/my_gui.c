...
static void keyboard_event_cb(lv_event_t *e)
{
    ...
    if (strcmp(txt,LV_SYMBOL_NEW_LINE) == 0)
    {
        ...
        if (select_flag == 1)
        {
            if (strcmp(name_txt,"lubenwei") == 0)
            {
                lv_label_set_text(label_name,LV_SYMBOL_OK);
            }
            else
            {
                lv_label_set_text(label_name,LV_SYMBOL_CLOSE);
            }
        }
        else if (select_flag == 2)
        {
            const char *password_txt = lv_textarea_get_text(ta_password);
            if (strcmp(password_txt,"123") == 0 && strcmp(name_txt,"lubenwei") == 0)
            {
                lv_label_set_text(label_password,LV_SYMBOL_OK);
            }
            else
            {
                lv_label_set_text(label_password,LV_SYMBOL_CLOSE);
            }
        }
    }
}
...
void my_gui(void)
{
    ...
    my_str_ta_t my_str_ta = {0};
    my_str_ta.width = scr_act_width()/2;
    my_str_ta.text_font = &lv_font_montserrat_14;
    my_str_ta.position = LV_ALIGN_CENTER;
    my_str_ta.x_ofs = 0;
    my_str_ta.y_ofs = -scr_act_height()/5;
    my_str_ta.one_line = 1;
    my_str_ta.max_length = 9;
    my_str_ta.placeholder_text = "user name";
    ta_name = my_ta_init(act,act,&my_str_ta);
    lv_obj_add_event_cb(ta_name,name_event_cb,LV_EVENT_ALL,NULL);

    label_name = label_text_create(act,ta_name,LV_ALIGN_OUT_RIGHT_MID,5,0,"",&lv_font_montserrat_16);

    my_str_ta.y_ofs = scr_act_height()/20;
    my_str_ta.position = LV_ALIGN_OUT_BOTTOM_MID;
    my_str_ta.password_mode = 1;
    my_str_ta.password_show_time = 1000;
    my_str_ta.max_length = 8;
    my_str_ta.placeholder_text = "password";
    ta_password = my_ta_init(act,ta_name,&my_str_ta);
    lv_obj_add_event_cb(ta_password,password_event_cb,LV_EVENT_ALL,NULL);

    label_password = label_text_create(act,ta_password,LV_ALIGN_OUT_RIGHT_MID,5,0,"",&lv_font_montserrat_16);

    keyboard = lv_keyboard_create(act);
