 ...
 static void cool_event_cb(lv_event_t *e)
{
    ...
    {
        cool_state = 1;
        lv_obj_set_style_img_recolor(target,lv_color_hex(0x00a9ff),LV_STATE_DEFAULT);
        lv_obj_set_style_img_recolor_opa(target,255,LV_STATE_DEFAULT);
    }
    else
    {
        cool_state = 0;
        lv_obj_set_style_img_recolor(target,lv_color_hex(0x8a8a8a),LV_STATE_DEFAULT);
        lv_obj_set_style_img_recolor_opa(target,255,LV_STATE_DEFAULT);
    }
}
...
void my_gui(void)
{
    ...
    my_str_obj.width = scr_act_width()-10;
    my_str_obj.height = scr_act_height()/2;
    my_str_obj.position = LV_ALIGN_CENTER;
    lv_obj_t *obj1 = my_obj_init(act,act,&my_str_obj);

    my_str_imgbtn_t my_str_imgbtn = {0};
    my_str_imgbtn.width = 64;
    my_str_imgbtn.height = 64;
    my_str_imgbtn.position = LV_ALIGN_LEFT_MID;
    my_str_imgbtn.x_ofs = scr_act_width()/7;
    my_str_imgbtn.y_ofs = -10;
    my_str_imgbtn.state = LV_IMGBTN_STATE_RELEASED;
    my_str_imgbtn.src_mid = &img_cool;
    lv_obj_t *imgbtn_cool = my_imgbtn_init(act,act,&my_str_imgbtn);
    label_text_create(act,imgbtn_cool,LV_ALIGN_BOTTOM_MID,0,30,"Cool",&lv_font_montserrat_22);
    lv_obj_add_event_cb(imgbtn_cool,cool_event_cb,LV_EVENT_PRESSED,NULL);

    my_str_imgbtn.position = LV_ALIGN_CENTER;
    my_str_imgbtn.x_ofs = 0;
    my_str_imgbtn.src_mid = &img_warm;
    lv_obj_t *imgbtn_warm = my_imgbtn_init(act,act,&my_str_imgbtn);
    label_text_create(act,imgbtn_warm,LV_ALIGN_BOTTOM_MID,0,30,"Warm",&lv_font_montserrat_22);
    lv_obj_add_event_cb(imgbtn_warm,warm_event_cb,LV_EVENT_PRESSED,NULL);

    my_str_imgbtn.position = LV_ALIGN_RIGHT_MID;
    my_str_imgbtn.x_ofs = -scr_act_width()/7;
    my_str_imgbtn.src_mid = &img_dry;
    lv_obj_t *imgbtn_dry = my_imgbtn_init(act,act,&my_str_imgbtn);
    label_text_create(act,imgbtn_dry,LV_ALIGN_BOTTOM_MID,0,30,"Dry",&lv_font_montserrat_22);
    lv_obj_add_event_cb(imgbtn_dry,dry_event_cb,LV_EVENT_PRESSED,NULL);

    my_vertical_line_create(act,LV_ALIGN_CENTER,-scr_act_width()/6,0,120,0x6c6c6c);
    my_vertical_line_create(act,LV_ALIGN_CENTER,scr_act_width()/6,0,120,0x6c6c6c);
